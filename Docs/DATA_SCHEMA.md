...
> **現在のスキーマバージョン: 2.1.0（episodes/構造＋多言語locales/。MINOR＝既存コンテナはlocales/を無視して動作可）**
...
│   │   │   ├── subtitles.srt
│   │   │   └── edit.json
│   │   └── locales/          ← ★多言語（2.1.0新設。詳細 §2c と Docs/08_i18n.md）
│   │       └── {lang}/       ←   例 en/ es/。言語別成果物のみ。footage/・a_roll/ は原語と共有
...
### episodes[].locales（多言語ステータス、2.1.0新設・任意キー）
...
```json
{
  "episodes": [{
    "number": 1,
    "title": "GPT-5発表の衝撃",
    "status": { "scripting": "done", "tts": "done", "footage": "done", "editing": "done", "video_edit": "pending" },
    "locales": {
      "en": {
        "title": "The Shock of GPT-5",
        "status": {
          "translation": "done",
          "tts": "pending",
          "editing": "not_started",
...
| フィールド | 意味 |
|---|---|
| `language`（トップレベル・既存） | **原語（正本言語）**として正式に意味付け（2.1.0）。既存プロジェクトは `"ja"` |
| `locales.{lang}.title` | 翻訳された話タイトル（publish用。翻訳時にLLMが生成） |
| `locales.{lang}.status.translation` | 行保存翻訳の工程状態（scripting-agentが書く） |
| `locales.{lang}.status.tts / editing / video_edit` | 原語と同じ意味の言語別工程。**`footage` は無い**（素材・Aロールは原語と共有のため） |

- 既存コンテナは `locales` キーを無視して動作できる（MINOR変更）。
- 原語 script.json を修正したら翻訳は「古い」扱い（§2c の `source_script_hash` で判定・UIで再翻訳を促す）。

### config.tts.locale_engines（言語→TTSエンジン解決、2.1.0新設・任意キー）

```json
{ "config": { "tts": { "engine": "irodori", "locale_engines": { "default": "omnivoice" } } } }
```

エンジン解決規則（tts-agent）: `lang == project.language` → `config.tts.engine`（現行動作・後方互換）／
それ以外 → `locale_engines[lang]` → `locale_engines.default` → 組み込み既定 `"omnivoice"`。
...
## 2c. locales/{lang}/script.json（行保存翻訳、2.1.0新設）

**場所:** `shared/projects/{project_id}/episodes/epNN/locales/{lang}/script.json`
**書き手:** scripting-agent（`POST /projects/{id}/episodes/{n}/translate`）
**読み手:** tts-agent（言語別TTS）、editing-agent（言語別OTIO/SRT）

原語の確定 `script.json`（§4）と**同スキーマ**に、翻訳メタ3フィールドを足したもの。
`line_id` / `order` / `speaker_id` / `section` / `emotion` / `speed` / `pause_after_sec` は**原本と同一**（行保存翻訳）。
`text` だけが対象言語になる。`notes` と `speaker_name` は翻訳しない（notesは制作メモ・speaker_nameは表示キャッシュで従来どおり都度解決）。
...
```json
{
  ...
  "locale": "en",
  "translated_from": "ja",
  "source_script_hash": "sha256(原語script.jsonの lines[].text を \n 連結した文字列)",
  "lines": [ /* 原本と同一構造。text のみ英語 */ ],
  "sections": [ /* 原本コピー */ ],
  "metadata": { /* 原本準拠 */ }
}
```
...
| フィールド | 意味 |
|---|---|
| `locale` | この翻訳の言語コード（ISO 639-1。例 `en` `es`） |
| `translated_from` | 原語（= project.json の `language`） |
| `source_script_hash` | 翻訳時点の原語script.jsonの内容ハッシュ。現在の原本と不一致なら「翻訳が古い」＝UIで再翻訳を促す |

**行保存翻訳の理由**: line_id/section構造を維持することで、Aロールパネル（`a_roll/`）と素材紐付け（`footage.json`）を
言語間でそのまま共有できる（再生成コストゼロ）。このため**ネイティブ生成（最初から外国語で台本生成）は行わない**。

### 声カタログの OmniVoice 用サイドカー（2.1.0新設・多言語TTSの前提）

```
shared/voices/irodori/
├── KUJO-OK.mp3           ← irodori用の元音声（従来どおり）
├── KUJO-OK.ref.wav       ← OmniVoice用の短尺(3-10秒)参照クリップ（文の区切りで切り出す）
└── KUJO-OK.ref.txt       ← .ref.wav の正確な書き起こし（UTF-8・原語）
```

- OmniVoice（多言語エンジン）はゼロショットクローンに `ref_audio + ref_text` を要求。**ref_text の省略・不正確は禁止**
  （省略するとサーバー内部で Whisper が VRAM 常駐し全生成が10倍劣化する実測済みの罠）。
- `.ref.*` が無い声で他言語TTSを実行した場合、tts-agent は 409 でブロックする（Phase 3実装）。
- 詳細・生成パラメータの必須値（`position_temperature=0` 等）は `Docs/08_i18n.md` §3c-3d を参照。
...
## 6c. edit.json（2026-06-13 新規・2.1.0で多言語対応）

**場所:** `shared/projects/{project_id}/episodes/epNN/edit/edit.json`（`lang`指定時は `episodes/epNN/locales/{lang}/edit/edit.json`）
**書き手:** editing-agent
...
| フィールド | 値 |
|---|---|
| `lang` | **2.1.0新規**。翻訳言語向けに生成した場合はそのコード（例`"en"`）、原語なら`null`。`files`のパスも連動して`locales/{lang}/edit/`配下になる |
...
| `gap_analysis.multiplier_ideas` | これを台本に足すと伸びやすい要素の提案（参考のみ・本文非反映） |
| `gap_analysis.title_patterns` | 伸びている動画のタイトルパターン例 |
| `for_script` | SEO指示文。仕様変更（2026-07-06）: 従来の全部盛り指示は参照用に残すが、台本生成時は使わない（代わりに`script_brief.keywords`だけを注入）。`script_brief`がメインの台本反映経路 |
| `script_brief` | **新規（2026-07-06）** パレット（`seo_pack`全体＝全SEO分析）からAIが厳選したキーワードだけを3〜6個に絞ったオブジェクト。以下の役割分担で使い分け（詳細は下のセクション参照）。`keywords`（配列）/ `curated_by`（`"ai"`または`"user"`）/ `curated_at`（ISO8601）を含む |
...
### パレット（seo_pack）とキュレーション層（script_brief）の役割分担（2026-07-06新規）

`seo_pack` はYouTube SEO分析全体（ジャンル推定・市場タグ・競合チャンネル・視聴者分析）を網羅するパレット（全カラー設定）です。一方 `script_brief` はこの分析パレットから、**この台本に自然に馴染むキーワードだけを3〜6個に厳選した台本本文への注入用データ**です。設計思想は「検索性より台本の面白さを優先」。

**各フィールドの使用場面：**
- `seo_pack.harvest.*`（タグ・チャンネル・下克上動画）: director-agent UI表示（参考・研究用）。**台本本文に直接反映しない**。
- `seo_pack.gap_analysis.multiplier_ideas`: 企画・差別化要素の提案。参考のみ、**本文非反映**。
- `seo_pack.for_script`: 従来のSEO指示文。この仕様変更により**参照用に残すが台本生成時は使わない**（非注入）。
- `seo_pack.script_brief.keywords`: **パレットから厳選した3〜6語だけ**。scripting-agent が台本生成プロンプトに注入する唯一のSEO方向性データ。これにより「キーワード詰め込みでシナリオが壊れる」を防ぐ。
- `publish_pack`（確定台本からのタイトル・概要欄・メタデータ）: 公開時。フルパレット（全SEO分析）を使い、メタデータの詰め込みは無害（視聴者には見えないため）。

**キュレーション動作：**
- 既定: `POST /projects/{id}/seo/optimize` 実行時、AIが自動で `script_brief` を軽く生成。
- 再選定: `POST /projects/{id}/seo/curate`（research-agent）でAI再選定（YouTube APIクォータ非消費）。
- 手動確定: `PATCH /projects/{id}/seo/brief` で UI からユーザーが手動選択。重複・空文字は自動除去、最大10個制限。`curated_by:"user"` でマーク。
